...
dis1 <- read.csv(paste(temp.dir,"Disturb_calcs_EX_rd_seis_500.csv",sep = ""))   # import distubance per layer
dis1 = dis1 %>% dplyr::select(-c(X))
# read in road layers 
dis3 <- read.csv(paste(temp.dir,"Max_temp_cals_500.csv",sep = ''))              # import road distubance
dis4 <- read.csv(paste(temp.dir,"Snake_temp_cals_500.csv",sep = ''))
dis5 <- read.csv(paste(temp.dir,"Chin_temp_cals_500.csv",sep = ''))
dis6 <- read.csv(paste(temp.dir,"West_temp_cals_500.csv",sep = ''))
dis7 <- read.csv(paste(temp.dir,"Cale_temp_cals_500.csv",sep = ''))
...
dis8 <- read.csv(paste(temp.dir,"Seismic_dist_all_herds_500.csv",sep = ''))
...
nat1 <- read.csv(paste(out.dir,"Nat_dist_type_RPC_500.csv",sep =""))  ; nat1 = nat1 %>% dplyr::select(-c(X))
nat2 <- read.csv(paste(out.dir,"Combines_Nat_dist_RPC_500.csv",sep ="")) 
...
dis10 <- read.csv(paste(out.dir,"Pest_by_type_RPC_500.csv",sep ="")) # pest types break down 
...
for (i in herds) { 
  ...
  dis9 <- read.csv(paste(out.dir,"Temporal_decade_dist_RPC_500.csv",sep ="")) 
  ...
  # write out results
  write.csv(r.area,paste(out.dir,i,"_Dist_Summary_ha_500.csv",sep = ''))
  write.csv(r.pc,paste(out.dir,i,"_Dist_Summary_pc_500.csv",sep = ''))
